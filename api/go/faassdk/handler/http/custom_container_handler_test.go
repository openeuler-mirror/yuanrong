...
func TestInitHandler(t *testing.T) {
	convey.Convey("InitHandler", t, func() {
		...
		convey.Convey("success", func() {
			dsClient := &fakeSDKClient{}
			patches := gomonkey.NewPatches()
			patches.ApplyFunc(sts.InitStsSDK, func(serverCfg types.StsServerConfig) error {
				return nil
			})
			patches.ApplyFunc(stsgoapi.DecryptSensitiveConfig, func(rawConfigValue string) (plainBytes []byte, err error) {
				return []byte{}, nil
			})
			patches.ApplyMethod(reflect.TypeOf(dsClient), "CreateClient", func(_ *fakeSDKClient, config api.ConnectArguments) (api.KvClient, error) {
				return &FakeDataSystemClinet{}, nil
			})
			patches.ApplyMethod(reflect.TypeOf(&basicHandler{}), "InitHandler", func(_ *basicHandler, args []api.Arg, apiClient api.LibruntimeAPI) ([]byte, error) {
				return nil, nil
			})
			defer patches.Reset()
			...
			res, _ := handler.InitHandler(args, dsClient)
			convey.So(res, convey.ShouldBeEmpty)
		})

		convey.Convey("DataSystem client init exception", func() {
			dsClient := &fakeSDKClient{}
			patches := gomonkey.NewPatches()
			patches.ApplyMethod(reflect.TypeOf(&basicHandler{}), "InitHandler", func(_ *basicHandler, args []api.Arg, apiClient api.LibruntimeAPI) ([]byte, error) {
				return nil, nil
			...
			patch0 := gomonkey.ApplyFunc(sts.InitStsSDK, func(serverCfg types.StsServerConfig) error {
				return errors.New("mock error for sts.InitStsSDK")
			})
			defer patch0.Reset()
			res, _ := handler.InitHandler(args, dsClient)
			convey.So(res, convey.ShouldBeEmpty)
			...
			patch0.Reset()
			patch0.ApplyFunc(sts.InitStsSDK, func(serverCfg types.StsServerConfig) error {
				return nil
			})

			patch1 := gomonkey.ApplyFunc(stsgoapi.DecryptSensitiveConfig, func(rawConfigValue string) (plainBytes []byte, err error) {
				return []byte{}, errors.New("mock error for stsgoapi.DecryptSensitiveConfig")
			})
			defer patch1.Reset()
			...
			patch1.Reset()
			patch1.ApplyFunc(stsgoapi.DecryptSensitiveConfig, func(rawConfigValue string) (plainBytes []byte, err error) {
				return []byte{}, nil
			})

			defer gomonkey.ApplyMethod(reflect.TypeOf(&fakeSDKClient{}), "CreateClient", func(_ *fakeSDKClient, config api.ConnectArguments) (api.KvClient, error) {
				return &FakeDataSystemClinet{}, errors.New("mock error for FakeDataSystemClinet.CreateClient")
			}).Reset()
