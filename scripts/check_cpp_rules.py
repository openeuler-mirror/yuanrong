...
def check_copyright_header(lines: List[str], file_path: str) -> List[Issue]:
    ...
    header = '\n'.join(lines[:20])
    has_copyright = any(re.search(pattern, header) for pattern in LICENSE_TEXT_PATTERNS)

    if not has_copyright:
        issues.append(Issue(file_path, 1, "G.CMT.03-CPP", "File header missing copyright notice"))
    return issues
...
def check_include_order(lines: List[str], file_path: str) -> List[Issue]:
    ...
    for i, line in enumerate(lines, 1):
        ...
        elif includes and stripped and not (
...
def main():
    ...
    if not files:
        LOGGER.info("No C++ files to check")
        return 0

    LOGGER.info("Checking %s files...", len(files))
