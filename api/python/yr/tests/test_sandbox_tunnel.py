...
class TestStartTunnelServerBehavior(unittest.TestCase):
    ...
    def test_successful_startup(self, mock_server_cls):
        ...
        mock_server_cls.assert_called_once_with(ws_port=19765, http_port=19766)

    @patch("yr.sandbox.tunnel_server.TunnelServer")
    def test_port_conflict_raises_runtime_error(self, mock_server_cls):
    ...
    @patch("yr.sandbox.tunnel_server.TunnelServer")
    def test_timeout_logs_warning_not_exception(self, mock_server_cls, mock_logger, mock_event_cls, mock_thread_cls):
