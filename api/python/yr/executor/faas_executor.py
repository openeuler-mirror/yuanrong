...
def faas_init_handler(posix_args: List[Any]) -> str:
    ...
    try:
        ...
        raise RuntimeError(
            transform_init_response_to_str(err_msg, FaasErrorCode.INIT_FUNCTION_FAIL)
        ) from e
    ...
    if user_init_code is None:
        raise RuntimeError(
            transform_init_response_to_str(
                f"failed to find init handler: {code_path}",
    ...
    _logger.info(
        "Succeeded to call FaaS user init handler: [%s]",
        context_meta["funcMetaData"]["handler"],
    )
    return transform_init_response_to_str("success", FaasErrorCode.NONE_ERROR)
...
def faas_call_handler(posix_args: List[Any]) -> str:
    ...
    if isinstance(event, dict):
        ...
        if not isinstance(header, dict):
            ...
            return transform_call_response_to_str(err_msg, error_code)
        header_trace_id = get_trace_id(header)
    ...
    try:
        ...
        _logger.exception(
            "Fail to run user call handler. err: %s. traceback: %s",
            exit_error,
    ...
    try:
        ...
        err_msg = (
            f"Fail to stringify user call result. "
        ...
        UserLogManager().shutdown()
    _logger.info(
        "Succeeded to call FaaS user call handler: [%s]",
        os.environ.get("RUNTIME_HANDLER", "handler name Not found in environment"),
    )
    return result_str
...
def faas_shutdown_handler(grace_period_second) -> ErrorInfo:
    ...
    error_info = ErrorInfo(
        ErrorCode.ERR_OK, ModuleCode.RUNTIME_KILL, "user function shutdown ok"
    )
    if user_code is None:
        ...
        while exit_loop:
            if requestQueue.empty():
                ...
                try:
                    ...
                    _logger.exception(err_msg)
                    error_info = ErrorInfo(
                        ErrorCode.ERR_USER_FUNCTION_EXCEPTION,
                        ModuleCode.RUNTIME,
                        err_msg,
                    )
...
# Helpers
def transform_call_response_to_str(
    response, status_code: FaasErrorCode, trace_id: str = ""
):
...
def transform_init_response_to_str(
    response, status_code: FaasErrorCode, trace_id: str = ""
):
    """Method transform_call_response_to_str"""
    result = dict(
        message="" if response is None else response, errorCode=str(status_code.value)
    )
    if trace_id:
        result["traceId"] = trace_id
...
def get_trace_id(headers: dict) -> str:
    """Get trace ID from request headers. Prefers X-Trace-Id, falls back to X-Request-Id."""
    return headers.get("X-Trace-Id") or headers.get("X-Request-Id") or ""
