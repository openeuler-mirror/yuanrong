...
RUN --mount=type=bind,source=openyuanrong_sdk-0.7.0.dev0-cp39-cp39-linux_x86_64.whl,target=/tmp/openyuanrong_sdk-0.7.0.dev0-cp39-cp39-linux_x86_64.whl \
...
RUN echo '#!/bin/bash' > /home/entryfile.sh && \
    ...
    echo 'export PYTHONPATH=./usr/local/lib/python3.9/site-packages/' >> /home/entryfile.sh && \
    echo 'exec ./usr/lib/x86_64-linux-gnu/ld-linux-x86-64.so.2 \' >> /home/entryfile.sh && \
    echo '    ./usr/local/bin/python3 \' >> /home/entryfile.sh && \
    echo '    ./usr/local/lib/python3.9/site-packages/yr/main/yr_runtime_main.py \' >> /home/entryfile.sh && \
    echo '    $@' >> /home/entryfile.sh && \
