...
func (f *fakeLeaseManager) CreateInstanceLease(insAlloc *types.InstanceAllocation, interval time.Duration,
	callback func()) (types.InstanceLease, error) {
	return &fakeInstanceLease{interval: interval}, nil
}

func (f *fakeLeaseManager) HandleInstanceDelete(instance *types.Instance) {}

func (f *fakeLeaseManager) CleanAllLeases() {}

type fakeInstanceLease struct {
	interval time.Duration
}

func (f *fakeInstanceLease) Extend() error {
	return nil
}

func (f *fakeInstanceLease) Release() error {
	return nil
}

func (f *fakeInstanceLease) GetInterval() time.Duration {
	return f.interval
}
...
func TestAcquireInstanceOtherQueue(t *testing.T) {
	bcs := newBasicConcurrencyScheduler(&types.FunctionSpecification{
		...
		queue.NewPriorityQueue(getInstanceID, priorityFuncForReservedInstance),
		queue.NewPriorityQueue(getInstanceID, priorityFuncForReservedInstance))
	bcs.leaseManager = &fakeLeaseManager{}
